...
These conventions were written for the domain models (`model.rs` and the
`precondition`/`invariant` modules). They may apply to other layers as well,
but that has not been established; treat them as defaults for model checks
until a second case proves otherwise.
